...
export default function GoForward() {
    ...
    const generateInitialPlatforms = useCallback(() => {
        ...
        platforms.push({
            x: 0,
            y: groundY,
            width: CONFIG.TILE_SIZE * 12,
            height: CONFIG.PLATFORM_HEIGHT,
            type: 'normal',
        });

        lastPlatformEndRef.current = CONFIG.TILE_SIZE * 12;
    ...
    const generateMorePlatforms = useCallback(() => {
        ...
        while (lastPlatformEndRef.current < viewRight + CONFIG.CANVAS_WIDTH) {
            ...
            let currentY = lastPlatform ? lastPlatform.y : CONFIG.CANVAS_HEIGHT - CONFIG.TILE_SIZE * 2;
            ...
            if (platformType === 'fake') {
                ...
                // 隨機決定往上還是往下
                if (Math.random() < 0.5) {
                    currentY = Math.max(CONFIG.TILE_SIZE * 2, currentY - fakeDiff * CONFIG.TILE_SIZE);
                } else {
            ...
            if (platformType === 'normal' && Math.random() < CONFIG.TUNNEL_CHANCE + difficultyFactor * 0.08) {
                ...
                if (ceilingY > CONFIG.TILE_SIZE * 2) {
                    platforms.push({
